...
router.get("/bodyPartList", (req, res) => {
  ...
  Exercises.map((exercise) => {
    bodyParts.indexOf(exercise.bodyPart) === -1 &&
...
router.get("/equipmentList", (req, res) => {
  const equipments = [];
  Exercises.map((exercise) => {
    equipments.indexOf(exercise.equipment) === -1 &&
...
router.get("/primaryMusclesList", (req, res) => {
  ...
  Exercises.map((exercise) => {
    primaryMuscles.indexOf(exercise.primaryMuscles) === -1 &&
      primaryMuscles.push(exercise.primaryMuscles);
  });
...
router.get("/amazonLinksList", (req, res) => {
  ...
  const amazonObj = Object.fromEntries(
    Exercises.map((exercise) => {
